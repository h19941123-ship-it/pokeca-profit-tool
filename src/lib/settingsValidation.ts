...
export const settingsFormSchema = z.object({
  ...
  bundleCards: z.coerce
    .number({ error: "まとめ発送枚数を数値で入力してください" })
    .int("まとめ発送枚数は整数で入力してください")
    .min(1, "まとめ発送枚数は1以上で入力してください")
    .max(50, "まとめ発送枚数は50以下で入力してください"),
...
export function parseSettingsForm(formData: FormData): SettingsParseResult {
  const raw = {
    ...
    bundleCards: formData.get("bundleCards"),
