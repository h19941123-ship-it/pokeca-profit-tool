...
- **相場ウォッチ** — 登録した銘柄のeBay出品価格を定点観測し、動いたものを知らせる
...
> ⚠️ **既定値の `1` から変えないのが安全です。**

同梱できるのは**同じ買い手が複数枚まとめて買ったとき**だけです。買い手が違えば
宛先が違うので、必ず1枚ずつ別便になります。「普段まとめて送っているか」ではなく
「1注文に何枚入るか」の設定である点に注意してください。

2以上にすると全カードの送料が一律で安く計算され、**利益が実際より多く見えます**。
仕入れ判断に使うと、買うべきでないカードを買う方向に外れます。

まとめ買いが常態化している場合にのみ変更してください。設定すると、送料
（100g 1枚=¥1,200 / 500g 5枚=¥2,040 → 1枚あたり¥408）に加えて、
**1注文あたりの費用も按分します**。
eBay定額手数料は1注文につき1回、購入者に請求する送料も1注文につき1回しか
受け取れないためです。
...
登録した銘柄の **eBay の出品価格** を定期的に記録し、上がってきたものを先頭に出します。
在庫として持っていないカードも監視できます（`Watch` は `Card` と独立）。

> ⚠️ 見ているのは **出品中の希望価格** であり、実際に売れた価格ではありません。
> 売り手が一斉に強気になっただけでも中央値は上がります。

そのため **出品件数** を併せて記録し、次の2つを区別します。

| 判定 | 条件 | 意味 |
|---|---|---|
| 買われている | 値上がり **かつ** 出品減 | 在庫が実際に吸われている可能性が高い |
| 値上がり | 値上がりのみ | 売り手が強気になっただけかもしれない |

比較は「直近3日」と「その前の11日」の**中央値どうし**で行います。1回の観測は
たまたま安い出品が1件あっただけで大きく振れるため、点と点では比べません。

観測は同じ銘柄につき **最短6時間おき**、1回の画面表示につき最大8件までです。
eBay API の利用上限に当たらないようにするためで、1日に何度取っても判断は変わりません。
「いま全部更新」を押すと間隔を無視して取り直します。

検索キーワードは **英語** で登録してください（eBay に日本語で問い合わせても当たりません）。

出品が0件のときは記録しません。0を中央値として残すと、次の観測で
「0ドルから急騰した」ことになってしまうためです。
